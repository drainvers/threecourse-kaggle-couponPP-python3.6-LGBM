"""
    This file creates submission file based on predicted values from lightgbm.
"""
...
predict_lgb = pd.read_csv("../model/lgb_predicted.txt", header=0, names=["prob"])
...
predict_df["predict_score"] = np.log( predict_lgb["prob"].values / (1 - predict_lgb["prob"].values) )
...
def create_answer(_predict_df):
    ...
    df = _predict_df.copy()
    df = df.sort_values(["final_score", "DISCOUNT_PRICE", "COUPON_ID"], ascending=[0,1,1])
    ...
    answer_df.to_csv("../model/predict_coupons_lgb.csv", index=False)
