"""
    This file creates input data for lgboost.
"""
...
def create_input_and_write(is_train):
    ...
    if is_train:
        ...
        lgbdata = create_input_multiprocess(train_ids)
        joblib.dump(lgbdata, '../model/lgbdata_train.pkl') 
        LOG.info("finished train write")
    else:
        test_ids = pd.read_pickle("../model/test_ids.pkl")
        lgbdata = create_input_multiprocess(test_ids)
        joblib.dump(lgbdata, '../model/lgbdata_test.pkl') 
...
def create_input_multiprocess(ids):
    ...
    lgbdfs = pd.concat(pool_results, ignore_index=True) 

    # release memory
    pool_results = None
    gc.collect()

    lgbdata = to_lgbdata(lgbdfs)
...
def to_lgb_df(_df):
    """Convert dataframe into lgb values dataframe."""

    LOG.info("start to_lgb_df")
...
def to_lgbdata(_df):
    """Convert lgb values dataframe into dictionary of labels, weights and data."""
    labels = _df.values[:,0]
